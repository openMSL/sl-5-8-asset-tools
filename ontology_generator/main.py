...
DATA_TYPE_MAP = {
    ...
    "state": {
        "type": XSD.string,
        "pattern": "^[a-zA-Z]{2}-(?:[a-zA-Z]{1,3}|[0-9]{1,3})$",
    },
    "link_type": {
        "type": XSD.string,
        "values": ["Image", "Video", "Document", "Routing", "Model"],
    },
    "hdmap_road_type": {
        "type": XSD.string,
        "values": [
    ...
    "scenario_type": {
        "type": XSD.string,
        "values": ["ASAM OpenSCENARIO XML", "ASAM OpenSCENARIO DSL"],
    },
    "scenario_participant_type": {
        "type": XSD.string,
        "values": ["car", "truck", "pedestrian"],
    },
    "model_type": {
        "type": XSD.string,
        "values": ["Unreal DataSmith", "Autodesk FBX", "OpenSceneGraph", "GLTF"],
    },
    "model_detaillevel_type": {"type": XSD.string, "values": ["High", "Medium", "Low"]},
    "geo_height_type": {
        "type": XSD.string,
        "values": ["Ellipsodial height", "Orthometric height"],
    },
}
...
# add to attributes
def addData(
    category: str, subcategory: str, data_name: str, data: dict, attributes: dict
):
        ...
        for sub_cat, sub_cat_data in cat_data.items():
            ...
            for attrib, attrib_data in sub_cat_data.items():
                ...
                if attrib_data["frequency"] == "1" or attrib_data["frequency"] == "1.0":
                    ...
                    logger.error(
                        f"unsupported frequency: {attrib_data['frequency']} for {attrib}"
                    )
...
def create_onotology(cat, cat_data, output_path, link_repro):
    ...
    ontology.add(
        (
            node,
            DCTERMS.contributor,
            Literal(f"{cat_data['categorie_data']['contributor']}"),
        )
    )
    ontology.add(
        (node, RDFS.label, Literal(f"ontology definition for {cat}", lang="en"))
    )
    ontology.add(
        (
            node,
            OWL.versionInfo,
            Literal(f"{cat_data['categorie_data']['version']}", datatype=XSD.float),
        )
    )
    ...
    ontology.add(
        (
            node_cat,
            RDFS.comment,
            Literal(f"{cat_data['categorie_data']['description']}", lang="en"),
        )
    )
...
def handle_data_type(root, propierty, cat_namespace, attrib_data):
    ...
    if data_type in DATA_TYPE_MAP:
        ...
        if "values" in type_data:
            ...
            values = (
                "(" + " ".join("'" + value + "'" for value in type_data["values"]) + ")"
            )
...
def create_property(root, shape, cat_namespace, attrib_data, order):
    ...
    if "description" in attrib_data:
        root.add(
            (propierty, SH.description, Literal(attrib_data["description"], lang="en"))
        )
...
def create_shacl(cat, cat_data, output_path, link_repro):
    ...
    for sub_cat, sub_cat_data in cat_data.items():
        ...
        for attrib, attrib_data in sub_cat_data.items():
            create_property(
                shacl, sub_cat_shape, cat_namespace, attrib_data, order_property
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="main.py",
        description="ontology and shacl files are generated from an excel table",
    )
    parser.add_argument(
        "-table", type=str, default="Metadata.xlsx", help="Path to Excel Table."
    )
    parser.add_argument(
        "-out",
        "--out",
        type=str,
        default="ontologies/",
        help="Path to exported ontology and shacl files.",
    )
    parser.add_argument(
        "-url",
        "--url",
        type=str,
        default="https://github.com/GAIA-X4PLC-AAD/map-and-scenario-data/tools/ontology_generator/ontologies/",
        help="URL for the ontologies.",
    )
