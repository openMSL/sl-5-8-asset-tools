...
from xodr_to_geojson_caller.models.junction import (
    Junction,
    JunctionConnection,
    JunctionLaneLink,
)
from xodr_to_geojson_caller.models.lane import (
    Lane,
    LaneHeight,
    LaneOffset,
    LaneSection,
...
from xodr_to_geojson_caller.models.opendrive import GeoReference, Header, OpenDRIVE
from xodr_to_geojson_caller.models.road import (
    ElevationProfile,
    LateralProfile,
    Polynomial,
    Road,
...
def _parse_lane_section(elem: etree._Element) -> LaneSection:
    ...
    left_lanes = [_parse_lane(l) for l in _findall(left_elem, "lane")] if left_elem is not None else []
    center_lane = _parse_lane(_findall(center_elem, "lane")[0]) if center_elem is not None and _findall(center_elem, "lane") else Lane(id=0)
    right_lanes = [_parse_lane(l) for l in _findall(right_elem, "lane")] if right_elem is not None else []
...
def _parse_object(elem: etree._Element) -> RoadObject:
    ...
    if outlines_elem is not None:
        for outline_elem in _findall(outlines_elem, "outline"):
            outlines.append(Outline(
                corner_road=[
                    CornerRoad(
                        s=_float(cr, "s"), t=_float(cr, "t"),
                        dz=_float(cr, "dz"), height=_float(cr, "height"),
                    )
                    for cr in _findall(outline_elem, "cornerRoad")
                ],
                corner_local=[
                    CornerLocal(
                        u=_float(cl, "u"), v=_float(cl, "v"),
                        z=_float(cl, "z"), height=_float(cl, "height"),
                    )
    ...
    repeats = [
        ObjectRepeat(
            s=_float(r, "s"), length=_float(r, "length"),
            distance=_float(r, "distance"),
            t_start=_float(r, "tStart"), t_end=_float(r, "tEnd"),
            width_start=_float(r, "widthStart"), width_end=_float(r, "widthEnd"),
            z_offset_start=_float(r, "zOffsetStart"), z_offset_end=_float(r, "zOffsetEnd"),
        )
        for r in _findall(elem, "repeat")
    ]

    return RoadObject(
        id=_str(elem, "id"), name=_str(elem, "name"), type=_str(elem, "type"),
        s=_float(elem, "s"), t=_float(elem, "t"), z_offset=_float(elem, "zOffset"),
        hdg=_float(elem, "hdg"), length=_float(elem, "length"),
        width=_float(elem, "width"), radius=_float(elem, "radius"),
        height=_float(elem, "height"), orientation=_str(elem, "orientation"),
        valid_length=_float(elem, "validLength"), subtype=_str(elem, "subtype"),
        outlines=outlines, repeats=repeats,
    )


def _parse_signal(elem: etree._Element) -> Signal:
    return Signal(
        id=_str(elem, "id"), name=_str(elem, "name"),
        type=_str(elem, "type"), subtype=_str(elem, "subtype"),
        s=_float(elem, "s"), t=_float(elem, "t"),
        z_offset=_float(elem, "zOffset"), hdg=_float(elem, "hOffset"),
        orientation=_str(elem, "orientation"),
        value=_float(elem, "value"), dynamic=_str(elem, "dynamic"),
...
def _parse_road(road_elem: etree._Element) -> Road:
    ...
    plan_view = [_parse_geometry(g) for g in _findall(plan_view_elem, "geometry")] if plan_view_elem is not None else []

    # ElevationProfile
    elev_elem = _find(road_elem, "elevationProfile")
    elevations = [_parse_polynomial(e) for e in _findall(elev_elem, "elevation")] if elev_elem is not None else []

    # LateralProfile
    lat_elem = _find(road_elem, "lateralProfile")
    super_elevations = [_parse_polynomial(se) for se in _findall(lat_elem, "superelevation")] if lat_elem is not None else []
    ...
    if lanes_elem is not None:
        lane_offsets = [
            LaneOffset(
                s=_float(lo, "s"), a=_float(lo, "a"),
                b=_float(lo, "b"), c=_float(lo, "c"), d=_float(lo, "d"),
            )
            for lo in _findall(lanes_elem, "laneOffset")
        ]
        lane_sections = [_parse_lane_section(ls) for ls in _findall(lanes_elem, "laneSection")]

    # Objects
    objects_elem = _find(road_elem, "objects")
    objects = [_parse_object(o) for o in _findall(objects_elem, "object")] if objects_elem is not None else []

    # Signals
    signals_elem = _find(road_elem, "signals")
    signals = [_parse_signal(s) for s in _findall(signals_elem, "signal")] if signals_elem is not None else []
...
def _parse_root(root: etree._Element) -> OpenDRIVE:
    ...
    geo_ref_elem = _find(header_elem, "geoReference") if header_elem is not None else None
    geo_ref = GeoReference(
        proj4=geo_ref_elem.text.strip() if geo_ref_elem is not None and geo_ref_elem.text else "",
    )
    header = Header(
    ...
    ) if header_elem is not None else Header(geo_reference=geo_ref)
