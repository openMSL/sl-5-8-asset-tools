...
def get_meta_data(file_data: dict, attributes: dict):
    if "project" in file_data:
        project_dict = dict()
        project_dict["environment-model:creationSource"] = file_data["project"][
            "creation_source"
        ]
        project_dict["environment-model:creationVersion"] = file_data["project"][
            "creation_version"
        ]
    ...
    if "quantity" in file_data:
        quantity_dict = dict()
        quantity_dict["environment-model:geometryCount"] = file_data["quantity"][
            "geometry_count"
        ]
        quantity_dict["environment-model:triangleCount"] = file_data["quantity"][
            "triangle_count"
        ]
        quantity_dict["environment-model:textureMaterialCount"] = file_data["quantity"][
...
def extract_meta_data(file: Path) -> Tuple[bool, dict]:
    ...
    data["shacl_type"] = (
        f"{get_schema_name().lower()}::{get_namespace()}#{get_schema_name()}Shape"
    )
