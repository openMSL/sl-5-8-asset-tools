...
def reduceXODR(box, file_in, file_out):
    ...
    for key, value in junctions.items():
        if not value[
            "inside"
        ]:  # all incomming road of this junctions are outside -> remove junction
            # get junction
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="main.py",
        description="removes the streets and intersections that are not in the specified bounding box and writes them out with *_reduce.xodr.",
    )
