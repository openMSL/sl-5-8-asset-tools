...
Assembles the final asset archive: organizes files into EVES-003 folder layout, generates the manifest attribute JSON, and produces the ready-to-ship asset.zip.

## Usage

```bash
python -m packager.main <input_manifest.json> -out <structure.json> -path <asset_dir> -asset_json <asset_instance.json> -asset_extractor <extractor.json>
```
