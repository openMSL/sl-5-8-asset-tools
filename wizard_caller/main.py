...
def post_filepath(file_path, endpoint_url, output_path=None):
    try:
        data = (
            {"file_path": file_path, "meta_data_location": output_path}
        ...
        if response.status_code == 200:
            ...
            if output_path is not None:
                print("and meta data location: " + output_path)
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="main.py",
        description="calls the sd creation wizard with json and merged shacl file to fill the non-extractable attributes from the user",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="if true call wizard and copy file to out, if false copy input to out",
    )
    parser.add_argument(
        "-out",
        type=str,
        required=True,
        help="output filename for enhanced json LD file",
    )
    ...
    if enable:
        ...
        post_filepath(
            str(jsonLD_file),
            "http://localhost:3000/processJsonLDFile",
            str(output_path),
        )
