...
Pipeline orchestrator. It executes configured extractor/creator modules, builds a temporary `asset.zip`, computes its CID, and renames the final archive to `<CID>.zip`.

## Usage

```bash
python -m pipeline.main <input_manifest.json> -config <config_dir> -out <output_dir> [-zip-dir <archive_dir>]
```
