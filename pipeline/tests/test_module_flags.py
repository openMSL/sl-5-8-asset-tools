...
class TestConfigFilenameToModuleId:
    def test_strips_prefix_and_suffix(self):
        assert (
            _config_filename_to_module_id("config_geojson_creator.json")
            == "geojson_creator"
        )

    def test_strips_prefix_and_suffix_simple(self):
        assert (
            _config_filename_to_module_id("config_metadata_extractor.json")
            == "metadata_extractor"
        )
...
class TestListModules:
    def test_returns_all_modules(self):
        ...
        assert "metadata_extractor" in ids
        assert "geojson_creator" in ids
        assert "packager" in ids
...
class TestListModulesCLI:
    def test_list_modules_exits_zero(self):
        ...
        assert "metadata_extractor" in result.stdout
...
from pipeline.main import _validate_asset_name
