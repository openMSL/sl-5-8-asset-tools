...
def validate_jsonld_against_shacl(
    data_graph: Graph, shacl_graph: Graph, json_LD_file: Path
):
...
def main():
    parser = argparse.ArgumentParser(
        prog="main.py", description="validate jsonLD against shacls"
    )
    parser.add_argument("filename", type=str, help="json LD filename")
    parser.add_argument(
        "-closed",
        action="store_true",
        help="set closed = true in all NodeShapes, to also check the naming of properties",
    )
